...
from core.keyboards.inline import get_inline_choose_pet_keyboard, get_inline_start_keyboard, gеt_go_menu_keyboard, \
    gеt_go_menu_location_keyboard
...
async def cmdStart(message: Message, request: Request):
...
async def askGeo(call: CallbackQuery, state: FSMContext):
    await state.set_state(user_status.application)
    await call.message.edit_reply_markup()
    await call.message.delete()
    global last_location_send
    last_location_send = await call.message.answer("Отправьте нам свою геолокацию!",
                                                   reply_markup=gеt_go_menu_location_keyboard())
...
async def determineCityByGeo(message: Message, state: FSMContext):
    ...
    last_message = await message.answer(
        'Вы отправили свою геолокацию! Можете отправить ее снова или вернуть в начальное меню\r\a'
...
@router.message(F.content_type == ContentType.WEB_APP_DATA, user_status.application)
async def getWebAppData(message: Message, state: FSMContext, request: Request):
    res = json.loads(message.web_app_data.data)
    await request.add_data_requests(res, message.from_user.id)
...
async def next_pet(call: CallbackQuery):
    ...
    await call.message.answer_photo(photo=url,
                                    caption=f'Name:  <a href=\'{url}\'><b>{name}</b></a>\nAge: <b>{age}</b>\nSex: <b>{sex}</b>\n{info[0:700]}',
                                    reply_markup=get_inline_choose_pet_keyboard())
